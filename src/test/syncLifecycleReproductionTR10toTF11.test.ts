  ...
  test("T-R15: RecoveryActionKind の全 Action について Policy → Executor → UI adapter の網羅的 parity が保証される", async () => {
    const allActions: Record<RecoveryActionKind, {
      ...
      retry_remote_write: {
        ...
        executorCall: async (engine, fixture) => engine.resolveTicketCommitUnknown({
          key: { kind: "ticket", ticketId: 801 },
          context: { connectionScope: SCOPE },
          attemptGeneration: fixture.op.attemptGeneration,
      ...
      link_remote_comment: {
        ...
        executorCall: async (engine, fixture) => engine.resolveCommentCommitUnknown({
      ...
      retry_effect: {
        ...
        executorCall: async (engine, fixture) => engine.resolveEffect({
          key: { kind: "newTicket", queueId: "q807" },
          operationId: `${SCOPE}:newTicket:q807`,
          operationRevision: 1,
          attemptGeneration: fixture.op.attemptGeneration ?? 1,
          effectId: "child-create:0",
