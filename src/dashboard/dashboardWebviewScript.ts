...
function renderSettingsBase(){
  ...
  element.innerHTML='<section class="settings-section"><h3>'+STRINGS.sectionConnection+'</h3>'+
    ...
    '<label class="setting-row" for="set-show-priority"><span class="setting-label">'+STRINGS.showPriorityLabel+'</span><input class="setting-check" id="set-show-priority" type="checkbox"'+(settings.showPriority?' checked':'')+'></label></section>'+
  ...
  document.getElementById('set-ticket-limit').addEventListener('change',function(){ const value=Number(this.value); if(value >= 1 && value <= 500) req('settings.updateGeneral',{patch:{ticketListLimit:value}}); }); document.getElementById('set-include-children').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{includeChildProjects:this.checked}}); }); document.getElementById('set-show-status').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{showStatus:this.checked}}); }); document.getElementById('set-show-due-date').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{showDueDate:this.checked}}); }); document.getElementById('set-show-tracker').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{showTracker:this.checked}}); }); document.getElementById('set-show-priority').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{showPriority:this.checked}}); }); document.getElementById('set-sync-mode').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{offlineSyncMode:this.value}}); });
