...
function actionIcon(name){
  const paths={cancel:'M6 6l12 12M18 6 6 18',open:'M3 7V5h6l2 3h10v12H3V7Z',comment:'M4 4h16v13H9l-5 4V4Z',sync:'M20 8a8 8 0 0 0-14-2L3 9m0-6v6h6M4 16a8 8 0 0 0 14 2l3-3m0 6v-6h-6'};
...
function renderSettingsBase(){
  ...
  element.innerHTML='<section class="settings-section"><h3>'+STRINGS.sectionConnection+'</h3>'+
    ...
    '<label class="setting-row" for="set-show-due-date"><span class="setting-label">'+STRINGS.showDueDateLabel+'</span><input class="setting-check" id="set-show-due-date" type="checkbox"'+(settings.showDueDate?' checked':'')+'></label></section>'+
  ...
  document.getElementById('set-ticket-limit').addEventListener('change',function(){ const value=Number(this.value); if(value >= 1 && value <= 500) req('settings.updateGeneral',{patch:{ticketListLimit:value}}); }); document.getElementById('set-include-children').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{includeChildProjects:this.checked}}); }); document.getElementById('set-show-status').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{showStatus:this.checked}}); }); document.getElementById('set-show-due-date').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{showDueDate:this.checked}}); }); document.getElementById('set-sync-mode').addEventListener('change',function(){ req('settings.updateGeneral',{patch:{offlineSyncMode:this.value}}); });
