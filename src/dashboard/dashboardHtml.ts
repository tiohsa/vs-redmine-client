  ...
  <section class="tab-panel active" id="panel-tickets" role="tabpanel" aria-labelledby="tab-tickets">
    <div class="tickets-layout">
      <div class="tickets-master">
        <div id="filter-bar"><div id="search-row"><div class="search-box"><label class="sr-only" for="search-input">${strings.searchPlaceholder}</label><svg class="action-icon search-icon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><circle cx="10" cy="10" r="6.5"/><path d="m15 15 5 5"/></svg><input id="search-input" type="text" role="searchbox" placeholder="${strings.searchPlaceholder}" autocomplete="off"><button id="search-clear-btn" class="search-clear-btn hidden" type="button" title="${strings.clearSearch}" aria-label="${strings.clearSearch}">×</button></div><label class="sr-only" for="ticket-layout-mode">${strings.layoutModeLabel}</label><select id="ticket-layout-mode" class="layout-mode-select" title="${strings.layoutModeLabel}" aria-label="${strings.layoutModeLabel}"><option value="auto">${strings.layoutAuto}</option><option value="single">${strings.layoutSingle}</option><option value="split">${strings.layoutSplit}</option></select></div><div class="quick-filter-row" role="group" aria-label="${strings.quickFiltersLabel}"><button class="quick-filter" data-quick-filter="mine" aria-pressed="false" type="button">${strings.quickMyIssues}</button><button class="quick-filter" data-quick-filter="open" aria-pressed="false" type="button">${strings.quickOpen}</button><button class="quick-filter" data-quick-filter="overdue" aria-pressed="false" type="button">${strings.quickOverdue}</button><button class="quick-filter" data-quick-filter="unsynced" aria-pressed="false" type="button">${strings.quickUnsynced}</button><button class="quick-filter" id="advanced-filters-btn" aria-haspopup="dialog" type="button">${strings.advancedFilters}</button></div><div class="filter-summary"><div id="filter-chips" aria-live="polite"></div><span id="ticket-count" class="ticket-count" role="status"></span></div></div>
...
<div id="sync-tray" class="sync-tray" role="status" aria-live="polite"></div>
