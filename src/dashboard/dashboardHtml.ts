...
<main id="content">
  ...
  <section class="tab-panel" id="panel-unsynced" role="tabpanel" aria-labelledby="tab-unsynced" hidden>
    <div id="unsynced-panel"><div class="unsynced-header"><div><h2 class="panel-title">${strings.tabUnsynced}</h2><p class="panel-subtitle" id="unsynced-count-label" aria-live="polite"></p></div><button id="sync-all-btn" class="btn btn-primary hidden" type="button">${dashboardActionIcon("sync")}<span>${strings.syncAllBtn}</span></button></div><div id="unsynced-summary" class="unsynced-summary" aria-live="polite"></div><div id="unsynced-feedback" class="operation-feedback hidden" role="status" aria-live="polite"></div><div id="unsynced-list" role="list"></div><button id="abandoned-toggle" class="btn btn-secondary hidden" type="button" aria-expanded="false" aria-controls="abandoned-list"></button><div id="abandoned-list" class="hidden" role="list"></div></div>
  </section>
