...
- Activity Bar に単一の**ダッシュボード Webview**（Tickets / Unsynced / Settings タブ）。コメントは選択中チケットの Work Panel に表示
...
拡張機能は Activity Bar に単一のダッシュボード Webview を提供し、3 つのタブで構成されます。
...
**Work Panel** にはチケットのメタデータ（ID・件名・プロジェクト・トラッカー・ステータス・優先度・日付・親チケット）と **概要 / コメント** タブが表示されます。コメントの編集は VS Code の Markdown エディタで行います。

クイックフィルター（自分のチケット、未完了、期限超過、未同期）は Dashboard に読み込み済みのチケットを絞り込みます。件数は表示中・読み込み済み・全件を区別します。**フィルター…** では永続的なチケットフィルターを編集できます。画面下部の同期トレイから既存の未同期・競合フローを開けます。

### Dashboard のメンテナンス

Settings > Maintenance の **Reset Dashboard Cache** は、Redmine から再構築可能な Dashboard データとメタデータを再取得します。ローカルの下書き、未同期の変更、復旧データ、認証情報、設定は保持されます。**Reset View State** は Dashboard のレイアウトとクイックフィルター設定をリセットします。永続的なチケット一覧設定は変更しません。
...
Redmine が作成・更新を受理した後も、remote read-back とローカル Markdown の確定が完了するまでは復旧待ちアイテムとして残ります。重複送信を防ぐ checkpoint を保持しているため、この状態のアイテムは通常の「破棄」では削除できません。書込み結果が不明なまま接続が切れた場合は自動再送せず、復旧が必要な状態として表示します。
