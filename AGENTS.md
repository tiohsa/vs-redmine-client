...
* Full current-generation compensation closes the current Attempt only after the Operation-level ledger confirms that every Effect is terminal-safe; Primary `compensated` alone is insufficient. At closure, its effects and remote identities are removed from the active set, the operation is queued with the next `attemptGeneration`, and callbacks/retries from the previous generation must be rejected without a persistence write.
